...
import { batchDeleteDogs } from "@/lib/actions/dogs";
...
export function DogListManager({ dogs }: { dogs: Dog[] }) {
  ...
  function handleDeleteSelected() {
    startTransition(async () => {
      await batchDeleteDogs([...selected]);
      exitSelectMode();
    });
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {!selectMode && (
            <Link href="/dogs/new" className={cn(buttonVariants({ size: "sm" }))}>
              新增狗狗
            </Link>
          )}
        </div>
          ...
          <div key={dog.id} className="relative">
            {selectMode && (
              <button
                ...
                aria-label={selected.has(dog.id) ? `取消選取 ${dog.name}` : `選取 ${dog.name}`}
              >
                {selected.has(dog.id) && (
            ...
            <div
              className={cn(
                "transition-opacity",
                selectMode && selected.has(dog.id) && "ring-2 ring-primary rounded-xl"
              )}
              onClick={selectMode ? () => toggleSelect(dog.id) : undefined}
            >
              <DogCard dog={dog} disableActions={selectMode} />
            </div>
          </div>
        ))}
      </div>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  確定要刪除選取的 {selected.size} 隻狗狗？此操作將同時刪除所有相關的照護、健康、花費記錄，且無法復原。
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
